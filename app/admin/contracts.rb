...
ActiveAdmin.register Contract do
  ...
  filter :box, as: :select, collection: Box.first(100).collect { |box| [box.full_name.to_s, box.id] }
  filter :client, as: :select, collection: Client.first(100).collect { |client| [client.full_name.to_s, client.id] }

  form do |f|
    f.semantic_errors(*f.object.errors.keys)

    f.inputs 'Details' do
      f.input :client_id, as: :select, collection: Client.first(100).collect { |client| [client.full_name, client.id] }
      f.input :box_id, as: :select, collection: Box.first(1000).collect { |box| [box.full_name.to_s, box.id] }
